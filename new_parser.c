...
int	add_cmd_lst_lst(char *list, cmd_lst_lst_t **head)
{
	...
	//printf("In add_cms_lst_lst node (%p)\n", node);
	//printf("In add_cmd_lst_lst *head (%p)\n", *head);
	if (!node)
		return (-1);
...
int	parse_logic_op(cmd_lst_lst_t **llav_head) // create the list of char**av -> linked list of cmds
{
	...
	//printf("In parse logic op llav_head (%p) *(%p)\n", llav_head, *llav_head);
	if (!llav_head)
		return (-1);
	node = *llav_head;
	while (node)
...
int	parse_ctrl_op(char *input, cmd_lst_lst_t **head) // create the list of char ***lav -> linked list of commands list
{
	...
	while (lists[i])
	{
		//printf("in parse op lists[%d] (%p) = [%s]\n", i, lists[i], lists[i]);
...
int	parser(char *input, cmd_lst_lst_t **llav_head)
{
	...
	if (parse_ctrl_op(input, llav_head) == -1)
		return (-1);
	//printf("Parce_strl_op passed\n");
	if (parse_logic_op(llav_head) == -1)
		return (-1);
	//printf("Parse_logic_op passed\n");
	...
	while(head)
	{
		...
		while (node)
		{
			printf("In parser node (%d) cmds: [%s] av: [%p] flag: [%c]\n",j, node->cmd,node->av, node->flag);
			node = node->next;
			++j;
		}
		head = (head->next);
